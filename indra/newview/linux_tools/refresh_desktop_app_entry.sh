...
function install_desktop_entry()
{
    ...
    local desktop_entry="\
[Desktop Entry]\n\
Name=Starbird Viewer\n\
Comment=Client for accessing 3D virtual worlds\n\
Exec=${installation_prefix}/starbird\n\
Icon=${installation_prefix}/starbird_icon.png\n\
Terminal=false\n\
Type=Application\n\
Categories=Application;Internet;Network;\n\
StartupNotify=true\n\
X-Desktop-File-Install-Version=3.0\n
StartupWMClass=do-not-directly-run-starbird-bin"

    echo " - Installing menu entries in ${desktop_entries_dir}"
    mkdir -vp "${desktop_entries_dir}"
    echo -e $desktop_entry > "${desktop_entries_dir}/starbird-viewer.desktop" || "Failed to install application menu!"
}
