...
{
	/// <summary>
	/// The valid brush numbers cannot be
	/// reordered, because they are used in
	/// the binary LSL format as arguments
	/// to llModifyLand().
	/// </summary>
	E_LANDBRUSH_LEVEL = 0,
	E_LANDBRUSH_RAISE = 1,
...
// Treated as 'unknown', usually ends up
// being SIM_ACCESS_PG.
constexpr U8 SIM_ACCESS_MIN = 0;
constexpr U8 SIM_ACCESS_PG = 13;
constexpr U8 SIM_ACCESS_MATURE = 21;

// Seriously ADULT ONLY!
constexpr U8 SIM_ACCESS_ADULT = 42;

// TODO:
// A Galaxy Beyond requires addtional ratings
// These will need to be added prior to
// their deploying the next generation
// of their platform.
...
/// GOD_MAINTENANCE = 250
/// 
/// This in fact is incorrect and
/// thus breaks the god levels and
/// god tools because developers in
/// OpenSimulator and its various forks
/// and offshoots including Halcyon
/// do not have correct information
/// relating to how this works.
/// 
/// OpenSimulator does not appear to
/// have much of a difference between
/// GOD_CUSTOMER_SERVICE and GOD_MAINTENANCE
/// and therefore there is no sense in
/// them having god tools.  Ideally they
/// should just use GOD_NOT and GOD_LIKE
/// 
/// WhiteCoreSim uses the following settings
/// 
/// GOD_NOT = 0
/// GOD_LIKE = 1
/// GOD_LIAISON = 100
/// GOD_CUSTOMER_SERVICE 150
/// GOD_FULL = 200
/// GOD_MAINTENANCE = 250
/// 
/// WhiteCoreSim has this partially correct
/// in that system accounts such as the
/// governor account or the shared inventory
/// account are system accounts and therefore
/// technically liaison accounts.  The system
/// accounts are supposed to be protected
/// system accounts and never used unless
/// absolutely necessary for proper functioning
/// of a grid.  Only a Grid Owner account should
/// have level 250 as a system account.
/// 
/// Linden Labs uses the following on Secondlife:
/// 
/// GOD_NOT = 0
/// GOD_LIKE = 1
/// GOD_CUSTOMER_SERVICE = 100
...
/// GOD_MAINTENANCE = 250
/// 
/// While it is not clear how they structure the
/// 'Linden' user accounts relating to god tools
/// and god levels, this is not actually correct.
/// However, for Secondlife this really does not
/// matter because you have to be an employee of
/// Linden Labs in order to have a 'Linden' staff
/// account.
/// 
/// All other platforms should really be using the
/// following:
/// 
/// GOD_NOT = 0
/// GOD_LIKE = 1
...
/// GOD_FULL = 250
/// 
/// Each platform should also provide abilities based
/// upon the actual god level of the account.  For
/// example, a Customer Service member of a virtual
/// world's staff should only need to be able to have the
/// ability to restart a region, help region and parcel
/// owners find scripts and objects which are being a
/// proper pain, or minimal region settings tasks.
/// 
/// An assistant developer is someone who helps with
/// developing and maintaining a virtual world's platform
/// code and servers so should have access to additional
/// debugging information similar to a core developer.
/// 
/// A core developer can be likened to a owner of a virtual
/// world who needs a higher level of the god tools that an
/// assistant developer may not need.
/// 
/// God tools also work with backend operations such as
/// handling support tickets, forum posts, etc.
/// </summary>
constexpr U8 GOD_FULL = 250;
constexpr U8 GOD_MAINTENANCE = 200;
constexpr U8 GOD_CUSTOMER_SERVICE = 150;
constexpr U8 GOD_LIAISON = 100;
...
// Alexandria Linden is the avatar (system account)
// that handles the ownership of
// shared assets and inventory in Secondlife.
LL_COMMON_API extern const LLUUID ALEXANDRIA_LINDEN_ID;

// Governor Linden is the owner
// of the mainland estate.
...
/// <summary>
/// Move these up so that we can hide them 
/// in "State" for object updates (for now)
/// </summary>
const U32 AGENT_ATTACH_OFFSET = 4;
const U32 AGENT_ATTACH_MASK = 0xf << AGENT_ATTACH_OFFSET;

/// <summary>
/// This method uses the upper and lower
/// nibbles to maintain backwards compatibility
/// with old objects that only used the upper
/// nibble
/// </summary>
